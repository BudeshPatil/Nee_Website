		...
		this.http.post<any>(apiUrl, requestBody).subscribe(
			(response) => {
				...
				if (response.code === 200 && response.result && response.result.length > 0) {
					...
					const title = product.meta_title || `${product.name} - Luxury Rental | Neelgund`;
					const description = product.meta_description || product.short_description || `Book ${product.name} rental in India with Neelgund.`;
					const keywords = product.meta_keywords || `${product.name}, luxury rental, India`;
					const image = product.image || 'https://www.neelgund.com/assets/logo.png';
					if (!environment.production) {
