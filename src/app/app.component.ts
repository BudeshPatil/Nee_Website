...
export class AppComponent implements OnInit {
	...
	@ViewChild('introDesktopVideo') introDesktopVideo!: ElementRef<HTMLVideoElement>;
	@ViewChild('introMobileVideo') introMobileVideo!: ElementRef<HTMLVideoElement>;
	...
	private routeToPageNameMap: { [key: string]: string } = {
		...
		'/product/list/rent-convertible-car-dubai': 'product/list/rent-convertible-car-dubai',
		'/product/list/sedan-car-for-rent-dubai': 'product/list/sedan-car-for-rent-dubai',
		'/product/list/rent-coupe-cars-dubai': 'product/list/rent-coupe-cars-dubai',
		'/product/list/suv-rentals-dubai': 'product/list/suv-rentals-dubai',
		'/product/list/van-rental-dubai': 'product/list/van-rental-dubai'
	};
	...
	private setSchemaForPage(pageName: string, canonicalUrl: string): void {
		...
		if (pageName === 'home') {
			...
			// 	{
			// 		question: 'How do I book a car at Ghost Rentals?',
			...
			// 		answer: 'Yes, Ghost Rentals offers luxury yacht rental services in Dubai. Please contact us for details.'
			// 	}
			// ]);
		...
		if (pageName === 'services') {
			this.schemaService.setLocalBusinessSchema({
				name: 'Ghost Rentals',
				description: 'Premium luxury car rental, chauffeur services, and yacht rentals in Dubai',
				phone: '+97180044678',
		...
		if (pageName === 'about') {
			this.schemaService.setLocalBusinessSchema({
				name: 'Ghost Rentals',
				description: 'Leading luxury car and yacht rental provider in Dubai since 2015',
				phone: '+97180044678',
				email: 'info@ghostrentals.com',
				city: 'Dubai',
				region: 'Dubai',
		...
		if (pageName.startsWith('product/list/')) {
			...
			this.schemaService.setCategorySchema(
				{
					name: categoryName,
					description: `Browse our collection of ${categoryName} available for rent in Dubai`
				},
				canonicalUrl
			);
	 ...
	 * Product detail routes are like: /product/rent-ferrari-296-gtb-dubai
	 */
	 ...
	 * From /product/rent-ferrari-296-gtb-dubai, extracts: rent-ferrari-296-gtb-dubai
	 */
	...
	private waitForMedia(): Promise<void> {
		return new Promise(resolve => {
			...
			if (this.introMobileVideo?.nativeElement) elements.push(this.introMobileVideo.nativeElement);
			if (this.introDesktopVideo?.nativeElement) elements.push(this.introDesktopVideo.nativeElement);

			const mobileThumb = new Image();
			mobileThumb.src = `${this.imageURL}/loader-video/rent-car-yacht-services-in-dubai.webp`;
